...
describe('table auto mode', () => {
    it('renders wide-containing tables as a nested bullet list (no pre)', () => {
        ...
        // Each row's first cell is bolded as the item title
...
describe('table records mode', () => {
    ...
    it('renders a header-only table as one unlabeled item', () => {
        const { text } = renderMarkdown('| 甲 | 乙 | 丙 |\n| --- | --- | --- |', {
            table: 'records',
        });
        expect(text).toBe(['• 甲', '    • 乙', '    • 丙'].join('\n'));
    });
});
